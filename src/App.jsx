...
const SECTION_WIDTHS = {
  ...
  about:      { mobile: 140, desktop: 50 },   
  education:  { mobile: 220, desktop: 60 },   
  experience: { mobile: 320, desktop: 80 },   
  skills:     { mobile: 300, desktop: 70 },   
  projects:   { mobile: 450, desktop: 120 },  
          ...
          <div className="relative h-full flex items-end pb-32 flex-shrink-0" style={getWidth('experience')}>
            ...
            <div className="flex gap-8 md:gap-16 ml-24 md:ml-24">
               <ExperienceCard 
                 ...
                 icon={Trophy} 
                 height="h-40"
                 description={`Worked remotely with the YugaYatra Retail web-development team, contributing full-stack features using HTML/CSS/JavaScript plus a backend stack. Helped with design and AI-related tasks, participated in code reviews, and improved documentation, teamwork, and time-management skills.`}
               />
               <ExperienceCard 
                 title="Research" 
                 ...
                 height="h-48" 
                 className="md:-mt-20"
                 description={`Won 1st place in an intensive international research bootcamp led by a Harvard–Cornell alumna. Built an ML-based waste segregation prototype that classifies recyclable vs non-recyclable waste to support automated, sustainable waste management.`}
               />
               <ExperienceCard 
                 title="SIH Hackathon" 
                 ...
                 icon={Zap} 
                 height="h-40"
                 description={`Participated in Smart India Hackathon with a solar–electrical hybrid power solution aligned with SDG-7 (Affordable & Clean Energy). Designed a sustainable energy prototype that reduces dependence on non-renewables and was shortlisted in the institute-level internal round.`}
               />
            </div>
          </div>
...
// Helper Component
const ExperienceCard = ({ title, role, color, icon: Icon, height, description, className = "" }) => {
    ...
    return (
        <div className={`relative flex flex-col items-center group flex-shrink-0 ${className}`}>
            ...
            <div className={`w-72 md:w-80 bg-slate-900/90 p-6 rounded-lg border-l-4 ${borderColors[color]} hover:bg-slate-800 transition-all transform hover:-translate-y-2 shadow-lg`}>
              ...
              {description && (
                <p className="mt-2 text-xs md:text-sm text-slate-400 leading-relaxed hidden group-hover:block">
                  {description}
                </p>
              )}
            </div>
